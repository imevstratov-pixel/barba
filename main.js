...
(() => {
  ...
  // Счётчик страницы: включается, только если в config.js задан GoatCounter.
  if (cfg.goatcounter) {
  ...
  function side(it) {
    ...
    const r = Math.min(textW(it.right), 190) + 26;
  ...
  function renderCompact(it, s, g) {
    const key = `${it.icon}|${it.tint}|${it.right}`;
    if (key !== lastKey) {
      compactEl.innerHTML = `<div class="nc-side nc-left t-${it.tint}">${ICON(it.icon)}<span class="${it.num ? 'num' : ''}"></span></div><div class="nc-gap"></div><div class="nc-side nc-right"><span></span></div>`;
      if (lastKey) pulse();
      lastKey = key;
    }
  ...
  const CARD_POSE = { task: 'approve', timer: 'stern', meeting: 'surprised', claude: 'happy', tg: 'lookup', tr: 'thinking' };
  ...
  if (!reduce && 'IntersectionObserver' in window) {
    const REVEAL = [
      ...
      ['.tries li', 70], ['.card', 60], ['.mini', 80], ['.perm', 70], ['.faq details', 50]
    ];
