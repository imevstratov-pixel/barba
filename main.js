...
(() => {
  ...
  // Локальный предпросмотр не считаем: иначе в Метрику попадают визиты самого автора.
  const localPreview = location.protocol === 'file:' || /^(localhost|127\.0\.0\.1)$/.test(location.hostname);
  if (cfg.metrika && !localPreview) {
  ...
  document.addEventListener('keydown', e => {
    ...
    if (open && (e.metaKey || e.ctrlKey) && /^[1-8]$/.test(e.key)) {
      e.preventDefault();
  ...
  const STATES = {
    ...
    claude: { icon: 'i-claude', left: 'ждёт ответа', right: 'Рефакторинг API', tint: 'accent' },
  ...
  const MINI = {
    ...
    claude: { icon: 'i-claude', l: 'Claude', r: 'ответил' },
    meeting: { icon: 'i-cal', l: '7 мин', r: 'Созвон' },
    limit: { icon: 'i-claude', l: 'лимит', r: 'обновился' }
  };
  ...
  if (dlBtn && cfg.downloadUrl) {
    ...
    dlBtn.addEventListener('click', () => {
      ...
      // Сразу показываем, как пройти предупреждение macOS: вопросы про «вредоносное ПО» приходят именно отсюда.
      setTimeout(() => document.getElementById('open-guide')?.scrollIntoView({ behavior: reduce ? 'auto' : 'smooth', block: 'center' }), 400);
